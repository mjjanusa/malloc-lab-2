...
int mm_init(void)
{
	...
	PUT(heap_listp + (3*WSIZE), 0); /* First Free Pointer  200 <= size < 400*/
	
	PUT(heap_listp + (4*WSIZE), 0); /* First Free Pointer  400 <= size < 600*/
	PUT(heap_listp + (5*WSIZE), 0); /* First Free Pointer  600 <= size < 800*/
	
	PUT(heap_listp + (6*WSIZE), 0); /* First Free Pointer  800 <= size < 1000*/
	PUT(heap_listp + (7*WSIZE), 0); /* First Free Pointer  1000 <= size < 1200*/
	
	PUT(heap_listp + (8*WSIZE), 0); /* First Free Pointer  1200 <= size < 1400*/
	PUT(heap_listp + (9*WSIZE), 0); /* First Free Pointer  1400 <= size < 1600*/
	
	PUT(heap_listp + (10*WSIZE), 0); /* First Free Pointer  1600 <= size < 1800*/
	PUT(heap_listp + (11*WSIZE), 0); /* First Free Pointer  1800 <= size < 2000*/
	
	PUT(heap_listp + (12*WSIZE), 0); /* First Free Pointer  2000 <= size < 2200*/
	PUT(heap_listp + (13*WSIZE), 0); /* First Free Pointer  2200 <= size < 2400*/
	
	PUT(heap_listp + (14*WSIZE), 0); /* First Free Pointer  2400 <= size < 2600*/
	PUT(heap_listp + (15*WSIZE), 0); /* First Free Pointer  2600 <= size < 2800*/
	
	PUT(heap_listp + (16*WSIZE), 0); /* First Free Pointer  2800 <= size < 3000*/
	PUT(heap_listp + (17*WSIZE), 0); /* First Free Pointer  3000 <= size < 3200*/
	
	PUT(heap_listp + (18*WSIZE), 0); /* First Free Pointer  3200 <= size < 3400*/
	PUT(heap_listp + (19*WSIZE), 0); /* First Free Pointer  3400 <= size < 3600*/
	
	PUT(heap_listp + (20*WSIZE), 0); /* First Free Pointer  3600 <= size < 3800*/
	PUT(heap_listp + (21*WSIZE), 0); /* First Free Pointer  3800 <= size < 4000*/
	
	PUT(heap_listp + (22*WSIZE), 0); /* First Free Pointer  4000 <= size < 4200*/
	PUT(heap_listp + (23*WSIZE), 0); /* First Free Pointer  4200 <= size/
...
////////////////////////////////////////////////////////////////
 static void *coalesce(void *bp)
 {
	...
	if (prev_alloc && next_alloc) { /* Case 1 */
		...
		if(GET(next) == 0 && GET(next + WSIZE) == 0) // if the prev free pointer and next free pointer were 0 set global first free pointer to 0.
 			PUT(heap_listp, 0); 	
 		else if (GET(next) == 0 && GET(next + WSIZE) != 0){// else if the prev pointer was 0 and next not zero make global first free pointer next.
 			PUT(heap_listp, GET(next + WSIZE));
 			PUT((char *)GET(next + WSIZE), 0);
 		}
 		else if (GET(next) != 0 && GET(next + WSIZE) == 0) // if prev pointer not 0 and next 0 then make prev's next pointer 0.
 			PUT(((char *)GET(next) + WSIZE), 0);
 		else {//if prev pointer and next pointer not 0 update pointers 
 			PUT(((char *)GET(next) + WSIZE), GET(next + WSIZE));	
 			PUT(((char *)GET(next + WSIZE)), GET(next));	
 		}
